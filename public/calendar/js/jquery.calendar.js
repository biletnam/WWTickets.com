	...
	({
		...
		{	
            ...
            var defaults = 
			{
				...
				eventText: 'Event: ',
				repetitiveEventActionText: 'Are you sure you want to delete entry?',
				...
				aspectRatio: 1, // will make day boxes bigger
			...
			({
				...
				eventRender: 
					...
					{	
						...
						var d_ticket = event.calTicketID;
						var d_custID = event.calCustID;
						...
						{	
							...
							element.attr('onclick', 'calendar.openModal("' + event.title + '","' + event.url + '","' + event.original_id + '","' + event.id + '","' + event.start + '","' + event.end + '","' + d_color + '","' + d_startDate + '","' + d_startTime + '","' + d_endDate + '","' + d_endTime + '","' + d_ticket + '","' + event.calCustID + '");');
						} 
					}	
				}); //fullCalendar
				
				 // Function to Open Modal
				calendar.openModal = function(title, url, id, rep_id, eStart, eEnd, color, startDate, startTime, endDate, endTime,calTicketID,calCustID)
				{					 
					 ...
					 calendar.id = id;
					 calendar.calTicketID = calTicketID;
					 calendar.calCustID = calCustID;
					 ...
					 ExpE = endDate+' '+endTime;
					  ...
					  $.ajax({
						...
						{ 
							...
							$('#edit-event').attr('href','edit_event.php?id=' + calendar.id).show();

							if(calendar.calTicketID == '') {
								$('#view-ticket').hide();
							}else{
								$('#view-ticket').attr('href','/tickets/ticket_view.php?ticketID=' + calendar.calTicketID + '&custID=' + calendar.calCustID).show();
							}
							...
							$('#add-event').hide();
							 ...
							 $('.modal-footer').show();
							 $(opt.modalSelector).modal('show').draggable();
						}
					  });
