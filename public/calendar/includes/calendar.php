...
//				var_dump("SELECT * FROM $this->table WHERE $this->condition $dc");
		...
		*/
		public function json_transform()
		{
			while($this->row = mysqli_fetch_array($this->result, MYSQLI_ASSOC))
			{
				 ...
				 $calTicketID = $this->row['calTicketID'];
				 $calCustID = $this->row['calCustID'];

				 $build_json = array('id' => $event_id, 'original_id' => $event_original_id, 'title' => $event_title, 'start' => $event_start, 'end' => $event_end, 'allDay' => $event_allDay, 'color' => $event_color, 'calTicketID' => $calTicketID, 'calCustID' => $calCustID);
		...
		*/
		...
		{
			$query = sprintf('SELECT id, title, description, start, end, allDay, color, url, category, user_id, calTicketID,calCustID FROM %s WHERE id = %d',
				  mysqli_real_escape_string($this->connection, $this->table),
				  mysqli_real_escape_string($this->connection, $id)
			);
...
//              Custom Fields Added by SDD - Power Pages
                $name = $extra['name'];
                $lname = $extra['lname'];
                ...
                $city = $extra['city'];
                $state = $extra['state'];
                $calUsers = $extra['calUsers'];
                $jobProduct = $extra['jobProduct'];
                $ticketStatus = $extra['ticketStatus'];
                $EntryNote = $extra['EntryNote'];
                $customerReferral = $extra['customerReferral'];
                $calTicketID = $extra['calTicketID'];
                $calCustID = $extra['calCustID'];
                $newCustomer = $extra['newCustomer'];


				# your own fields would be: $field = $extra['field_name']; and add them below on the $query as others are
				
				if(strlen($category) == 0) { $category = 'General'; }
				if(strlen($user_id) == 0) { $user_id = 0; }

                ###########################################
                # ~ Scott DeBoer - 16.08.31
                #   The following code is modified/added to insert the data into other database tables
                ###########################################
                ...
                if ($newCustomer == '1') {
                    ...
                    $databaseInsertedTable = 'customerInfo';

                    // The Basic Database - Add Event Query
                    // Insert into the customer info database
                    $query = sprintf('INSERT INTO %s SET 
                        ...
                        storeID = "%s"
                    ',
                        mysqli_real_escape_string($this->connection, $databaseInsertedTable),
                        mysqli_real_escape_string($this->connection, htmlspecialchars($name)),
                        mysqli_real_escape_string($this->connection, htmlspecialchars($lname)),
                        mysqli_real_escape_string($this->connection, htmlspecialchars($address)),
                        mysqli_real_escape_string($this->connection, htmlspecialchars($city)),
                        mysqli_real_escape_string($this->connection, htmlspecialchars($state)),
                        mysqli_real_escape_string($this->connection, htmlspecialchars($phone)),
                        mysqli_real_escape_string($this->connection, htmlspecialchars($email)),
                        mysqli_real_escape_string($this->connection, htmlspecialchars($EntryNote, ENT_COMPAT, 'UTF-8')),
                        mysqli_real_escape_string($this->connection, htmlspecialchars($custRank)),
                        mysqli_real_escape_string($this->connection, htmlspecialchars($storeID))
                    );

                    // The result
                    $this->result = mysqli_query($this->connection, $query);

                    $custID = mysqli_insert_id($this->connection);

                    $databaseInsertedTable = 'customerTickets';
                    ...
                    // The Basic Database - Add Event Query
                    // Unsert into customer tickets database
                    $query = sprintf('INSERT INTO %s SET 
                        ticketCustID = "%s",
                        propertyType = "%s",
                        ticketCity = "%s",
                    ...
                    ',
                        mysqli_real_escape_string($this->connection, $databaseInsertedTable),
                        mysqli_real_escape_string($this->connection, htmlspecialchars($custID)),
                        mysqli_real_escape_string($this->connection, htmlspecialchars($propertyType)),
                        mysqli_real_escape_string($this->connection, htmlspecialchars($ticketCity)),
                        mysqli_real_escape_string($this->connection, htmlspecialchars($ticketState)),
                        mysqli_real_escape_string($this->connection, htmlspecialchars($ticketStatus)),
                        mysqli_real_escape_string($this->connection, htmlspecialchars($ticketDate))
                    );
                ...
                ###########################################
                #End of modification - Presume with built function
                ###########################################
				...
				$query = sprintf('INSERT INTO %s 
							...
							',
											...
											mysqli_real_escape_string($this->connection, $description),
											...
											mysqli_real_escape_string($this->connection, htmlspecialchars($city)),
											mysqli_real_escape_string($this->connection, htmlspecialchars($state)),
											mysqli_real_escape_string($this->connection, htmlspecialchars($calUsers)),
											mysqli_real_escape_string($this->connection, htmlspecialchars($jobProduct)),
											mysqli_real_escape_string($this->connection, htmlspecialchars($ticketStatus)),
                                            mysqli_real_escape_string($this->connection, htmlspecialchars($EntryNote, ENT_COMPAT, 'UTF-8')),
                                            mysqli_real_escape_string($this->connection, htmlspecialchars($customerReferral)),
                                            mysqli_real_escape_string($this->connection, htmlspecialchars($calTicketID)),
                                            mysqli_real_escape_string($this->connection, htmlspecialchars($newCustomer)),
                                            mysqli_real_escape_string($this->connection, htmlspecialchars($calCustID))
                                                    );
...
//                This updates the database with the repeat_id - set to "1" by SDD - PPGS - 16.08.26
//					sprintf("UPDATE %s SET repeat_id = '1' WHERE id = %d", mysqli_real_escape_string($this->connection, $this->table), $inserted_id, $inserted_id)
				);
